...
SELECT p.PhoneNumber, t.Name type
FROM Person.PersonPhone AS p INNER JOIN
     Person.PhoneNumberType AS t ON p.PhoneNumberTypeID = t.PhoneNumberTypeID
--WHERE p.BusinessEntityID BETWEEN 1 AND 10

SELECT p.PhoneNumber, 
    (SELECT t.Name FROM Person.PhoneNumberType t WHERE p.PhoneNumberTypeID = t.PhoneNumberTypeID) type
...
ALTER DATABASE AdventureWorks2025 SET QUERY_STORE CLEAR;
